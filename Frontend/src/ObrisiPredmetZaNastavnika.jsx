...
function ObrisiPredmetZaNastavnika({ nastavnici, predmeti, onClose, onDeleted }) {
  const [selectedNastavnik, setSelectedNastavnik] = useState("");
  const [selectedPredmet, setSelectedPredmet] = useState("");

  const predmetiZaNastavnika = predmeti[selectedNastavnik] || [];

  const handleDelete = async () => {
    if (!selectedNastavnik || !selectedPredmet) {
    ...
    try {
      await axios.delete(
        `/api/nastavnik/nastavnici/${selectedNastavnik}/predmeti/${selectedPredmet}`
      );
      alert("Predmet uspešno obrisan za izabranog nastavnika!");
      onDeleted();
      onClose();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="modal-overlay">
      <div className="modal">
        <h3>Obriši predmet za nastavnika</h3>

        <div style={{ marginBottom: "10px" }}>
          <select
            value={selectedNastavnik}
            onChange={(e) => {
              setSelectedNastavnik(e.target.value);
              setSelectedPredmet("");
            }}
          >
        ...
        <div style={{ marginBottom: "10px" }}>
          <select
            ...
            disabled={!selectedNastavnik || predmetiZaNastavnika.length === 0}
          >
            <option value="">-- Izaberite predmet --</option>
            {predmetiZaNastavnika.map((p) => (
...
export default ObrisiPredmetZaNastavnika;
