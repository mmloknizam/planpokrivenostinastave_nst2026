...
const PokrivenostNastaveForm = ({ onCancel, onSuccess }) => {
  ...
  return (
    <div style={{ maxWidth: "500px", margin: "20px auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <label>Predmet:</label>
        <select name="predmetID" value={formData.predmetID} onChange={handleChange} required>
          <option value="">-- Izaberi predmet --</option>
          {predmeti.map(p => (
            <option key={p.predmetID} value={p.predmetID}>{p.naziv}</option>
          ))}
        </select>
        <br /><br />

        <label>Oblik nastave:</label>
        <select name="oblikNastaveID" value={formData.oblikNastaveID} onChange={handleChange} required>
          <option value="">-- Izaberi oblik nastave --</option>
          {obliciNastave.map(o => (
            <option key={o.oblikNastaveID} value={o.oblikNastaveID}>{o.tip}</option>
          ))}
        </select>
        <br /><br />

        <label>Školska godina:</label>
        <select name="skolskaGodinaID" value={formData.skolskaGodinaID} onChange={handleChange} required>
          <option value="">-- Izaberi godinu --</option>
          {skolskeGodine.map(g => (
            <option key={g.skolskaGodinaID} value={g.skolskaGodinaID}>{g.godina}</option>
          ))}
        </select>
        <br /><br />

        <label>Nastavnik:</label>
        <select name="nastavnikID" value={formData.nastavnikID} onChange={handleChange} required>
          <option value="">-- Izaberi nastavnika --</option>
          {nastavnici.map(n => (
            <option key={n.nastavnikID} value={n.nastavnikID}>{n.ime} {n.prezime}</option>
          ))}
        </select>
        <br /><br />
